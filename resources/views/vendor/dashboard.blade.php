...
<head>
    ...
    <title>{{ __('messages.be_vender') }} - Affican Digital</title>
    ...
    <div class="flex flex-col md:flex-row min-h-screen">
        <aside class="w-full md:w-64 bg-gray-900/50 border-{{ app()->getLocale() == 'ar' ? 'l' : 'r' }} border-gray-800 p-6">
            ...
            <nav class="space-y-4">
                ...
                <a href="#" class="block text-[#0077FF] font-bold">📊 {{ __('messages.stats') }}</a>
                <a href="#" class="block text-gray-400 hover:text-white">📚 {{ __('messages.your_books') }}</a>
            </nav>
        </aside>
        ...
        <main class="flex-1 p-4 md:p-10">
            <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                ...
                <div class="bg-gray-900 p-4 rounded-2xl border border-[#FFD700]/20 shadow-xl">
                    <p class="text-xs text-gray-500 uppercase tracking-widest mb-1">{{ __('messages.net_profit') }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                ...
                <div class="bg-gray-900/30 p-6 rounded-2xl border border-gray-800">
                    <p class="text-gray-500 text-sm">{{ __('messages.digital_works') }}</p>
